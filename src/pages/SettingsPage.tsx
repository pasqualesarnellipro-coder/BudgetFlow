  ...
  return (
    <div className="p-6 max-w-2xl space-y-5">
      ...
      <Card title={t('settings_language')}>
        ...
        <p className="text-xs text-gray-400 mt-2">La lingua si applica a tutta l'interfaccia dell'app.</p>
      </Card>

      <Card title={t('settings_name')}>
        <div className="flex gap-3">
          <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
            className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-income/50" />
        ...
        <div className="flex gap-2 flex-wrap">
          {([
            ...
            <button key={p.value} onClick={() => handleSave({ profile_type: p.value })}
              className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors ${form.profile_type === p.value ? 'bg-income border-income text-white' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
              {p.label}
            </button>
          ))}
        </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {([
                ...
                <button key={r.value} onClick={() => handleSave({ tax_regime: r.value })}
                  className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors ${form.tax_regime === r.value ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                  {r.label}
                </button>
              ))}
            </div>
          ...
          <Card title="Coefficiente ATECO" help="Il coefficiente di redditività ATECO determina l'imponibile fiscale sul quale vengono calcolate tasse e INPS. Ogni categoria di attività ha il suo coefficiente stabilito per legge.">
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="flex-1">
                <p className="text-xs text-gray-500 mb-1">Coefficiente attuale</p>
              ...
              <div className="text-right">
                <p className="text-xs text-gray-400">Imponibile su</p>
                <p className="text-sm font-semibold text-gray-700">€ 10.000 lordi</p>
      ...
      {form.profile_type === 'BOTH' && (
        ...
        >
          ...
          {/* Dove lo trovo */}
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3.5 mb-3 space-y-2">
            ...
            {[
              ...
              <div key={label} className="flex gap-2">
                ...
                <div>
                  ...
                  <span className="text-xs text-amber-600 ml-1">{detail}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {isFreelance && (
        <div>
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Integrazioni</h3>
          <FICConnect />
        </div>
      )}
