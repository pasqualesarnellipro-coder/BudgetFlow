...
export function AnnualBudgetPage() {
  ...
  // ── Apri pannello fill per una riga ──────────────────────────────────────
  const openFill = (cat: Category) => {
    // Pre-compila con il primo valore non-zero della riga (o stringa vuota)
    const firstNonZero = MONTH_NAMES.findIndex((_, i) => getAmount(cat.id, i + 1) > 0)
    const suggestedAmount = firstNonZero >= 0 ? String(getAmount(cat.id, firstNonZero + 1)) : ''
    setFillPanel({
      catId: cat.id,
      catName: cat.name,
      icon: cat.icon,
              ...
              <table className="w-full text-sm min-w-max">
                <thead>
                  <tr className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide border-b border-gray-50 dark:border-gray-700">
                    ...
                    {/* Colonna fill — sempre presente, contenuto visibile solo su hover riga */}
                    <th className="w-8" />
                  </tr>
                </thead>
                <tbody>
                  {typeCats.map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {MONTH_NAMES.map((_, i) => {
                          ...
                          return (
                            <td key={i} className="px-2 py-1">
                              <input
                                // key forza re-mount dopo fill → defaultValue aggiornato
                                key={`${cat.id}-${month}-${currentAmount}`}
                                type="number"
                                defaultValue={currentAmount || ''}
                                onBlur={(e) => handleBlur(cat.id, month, e.target.value)}
                                placeholder="0"
                                className="w-full text-right text-xs border border-transparent hover:border-gray-300 dark:hover:border-gray-500 focus:border-income rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-income/50 bg-transparent dark:text-gray-200"
                              />
                            </td>
                          )
                        })}
                        ...
                        {/* Pulsante fill — visibile on hover o quando il pannello è aperto per questa riga */}
                          ...
                          <button
                            ...
                            className={`p-1.5 rounded-lg transition-all ${
                              isActiveFill
                                ...
                                : 'text-gray-300 dark:text-gray-600 opacity-0 group-hover:opacity-100 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'
                            }`}
                          >
                            <CopyPlus size={14} />
                          </button>
